...
- **QuickBite web app** (Next.js, Cloud Run/Firebase) — the primary build. Disguised ordering UI, Gemini Live integration via WebSocket from the browser, structured extraction, leakage-check pass. Satisfies the mandatory deployed-link requirement directly, fastest to build, and arguably a MORE convincing disguise than a native app (zero install footprint — just a browser tab).
