...
- **Web app** (primary build): plain React (Vite), deployed on Cloud Run/Firebase — satisfies the mandatory deployed-link requirement, fastest to prove the core mechanism, and arguably the most convincing disguise (zero install footprint). Plain React, not Next.js — no SEO/server-rendering need for a client-heavy disguise UI + WebSocket session.
...
- **Phase 2 exception (user's explicit decision, 2026-09-25):** the heart double-tap silent SOS (`docs/backlog.md` Epic 11) records from **both front and back cameras**. The live call (Epics 9–10) stays back-camera only. Do not push back on the SOS front camera as a rule violation.
